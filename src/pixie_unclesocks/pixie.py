    ...
    def abuse_lookup(self, ip_list):
        ...
        for index, ip in enumerate(ip_list, start=1):
            ...
            if not public_address_parser(ip):
                ...
                isp = "NOT APPLICABLE"
                abuse_raw_score = 0
                abuse_score = "NOT APPLICABLE"
                total_reports = "NOT APPLICABLE"
...
class FilterLogic():
    ...
    def build_filter(self):
        ...
        filter_pattern = re.compile(r'((((?P<filter_key_int>CONFIDENCE|TOTALREPORTS)(?:\s*)?(?P<filter_op_int>>=|<=|==|!=|>|<|=))|((?P<filter_key_str>ISP|COUNTRY)(?:\s*)?(?P<filter_op_str>contains|!contains)))(?:\s*)?(?P<filter_value>\S+))', 
                                    re.IGNORECASE)
        ...
        for filter in self.filter_args:
            ...
            key = filter_match.group('filter_key_int') or filter_match.group('filter_key_str')
            op = filter_match.group('filter_op_int') or filter_match.group('filter_op_str')
            ...
            try:
                ...
                raise ValueError(f"ERR-FL02: Invalid operator. Use --help option for more information on the available operators.")

            try:
                value = cast(value)
            except:
               raise ValueError(f"ERR-FL03: Invalid {value} cast value. Cast must be {cast}.") 
